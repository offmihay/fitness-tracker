...
export const HeaderProvider = ({ children }: Props) => {
  const [headerOptions, setSettings] = useState<HeaderOptions>(defaultOptions);
  ...
  const updateSettings = async (newSettings: Partial<Settings>) => {
    try {
      const storeSettings = { ...headerOptions };
  ...
  return (
    <SettingsContext.Provider value={{ settings: headerOptions, updateSettings }}>
      {children}
